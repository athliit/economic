...
class Line
{
    ...
    {
        return $this->client->OrderLine_GetDataArray([
            'entityHandles' => $handles
        ])
        ->OrderLine_GetDataArrayResult
        ->OrderLineData;
    }
    ...
    {
        if (is_object($product) and isset($product->Number)) {
            return $product;
        }

        $productHandle = $this->client->Product_FindByNumber([
            'number' => $product
        ])->Product_FindByNumberResult;

        $this->client->OrderLine_SetProduct([
    ...
    {
        $this->client->OrderLine_SetDescription([
            'orderLineHandle' => $orderLineHandle,
            'value'           => $description
        ]);
    ...
    {
        $this->client->OrderLine_SetUnitNetPrice([
            'orderLineHandle' => $orderLineHandle,
            'value'           => $price
        ]);
    ...
    {
        $this->client->OrderLine_SetQuantity([
            'orderLineHandle' => $orderLineHandle,
            'value'           => $qty
        ]);
    ...
    {
        $unitHandle = $this->client->Unit_FindByNumber([
            'number' => $unit
        ])->Unit_FindByNumberResult;

        $this->client->OrderLine_SetUnit([
            'orderLineHandle' => $orderLineHandle,
